(function(){
  ...
  H5lock.prototype.update = function(po) {// 核心变换方法在touchmove时候调用
    ...
    this.drawStatusPoint("#27AED5");// 每帧花轨迹,圆形轮廓

    this.drawLine("#27AED5", po, this.lastPoint);// 每帧画直线
  ...
  H5lock.prototype.storePass = function(psw) {// touchend结束之后对密码和状态的处理
    if (this.pswObj.step == 1) {
      ...
      if (this.checkPass(this.pswObj.spassword, psw)) {
        ...
        this.drawStatusPoint("red");
        this.drawPoint("red");
        this.drawLine("red",this.lastPoint[this.lastPoint.length-1] , this.lastPoint);// 每帧画圆心x
        var title = document.getElementById("title");
        title.style.color = "red";
