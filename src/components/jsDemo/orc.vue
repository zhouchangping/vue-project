<template>
  <div>
    <canvas ref="canvas" width="880" height="1500"></canvas>
  </div>
</template>
<script>
export default {
  data() {
    return {
      url: require("@/common/images/images.png"),
    };
  },
  mounted() {
    var image = new Image();
    image.onload = this.recognition();
    image.src = "@/common/images/images.png";
    // 输出耗时
    console.log(this.count, (new Date().getTime() - this.beginTime) / 1000 + " s");
    // 将文字绘制到图片对应位置上，以方便查看提取是否正确
    this.context.drawImage(this, this.width, 0);
    var textArray = this.result.split("\n");
    for(var i = 0; i < textArray.length; ++i){
      this.context.fillText(textArray[i], this.width + this.beginX, this.beginY + this.lineHeight * i);
    }
  },
  methods: {
    recognition(){
      ...
      for(var i = 0; i < letters.length; ++i){
        ...
        // 获取字符绘制宽度
        var width = this.context.measureText(letter).width;
        // 绘制白色背景，与图片背景对应
        this.context.fillStyle = "#fff";
        this.context.fillRect(0, 0, width, 22);
        // 绘制文字，以获取字型数据
        ...
        this.letterData[letter] = {
          width : width,
          data : this.getBinary(this.context.getImageData(0, 0, width, 22).data)
        };
        // 清空该区域以获取下个字符字型数据
        this.context.clearRect(0, 0, width, 22);
      }
      ...
      // let y = 161;
      // 递归开始
      this.findLetter(this.beginX, this.beginY, "");
    },
    // 递归函数
    findLetter(x, y, str){
      // 找到结果文本，则递归结束
      ...
      var queue = [];
      // 循环匹配字符库字型数据
      for(var letter in this.letterData){
        // 获取当前字符宽度
        var width = this.letterData[letter].width;
        // 获取该矩形区域下的灰度化0-1数据
        var data = this.getBinary(this.context.getImageData(x, y, width, 22).data);
        ...
        for(var i = 0, l = data.length; i < l; ++i){
          ...
          // 不匹配的像素点，偏差量自增1
          if(data[i] != this.letterData[letter].data[i]){
            ++deviation;
          }
        }
        ...
        // 如果匹配完所有17行数据，则递归结束
        if(y > this.beginY +  this.lineHeight * 17){
          this.result = str;
          break;
        }
        // 如果已经到了行末，重置匹配坐标
      ...
      if(queue.length){
        ...
        // console.log(queue);
        // 从队头开始进行下一个字符的匹配
        for(var j = 0; j < queue.length && ! this.result; ++j){
          var item = queue[j];
          // 下一步递归
          this.findLetter(x + item.width, y, str + item.letter);
        }
      }else{
        return false;
      }
